...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadCustomerPreview(id) {
    ...
    fetch(`/customers/${id}.json`).then((res) => res.json()).then((c) => {
      ...
      card.innerHTML = `
        ...
        `;
      if (!existing) {
        const previewCardBody = document.querySelector(".card:has(#customer-history) .card-body") || historyBox.closest(".card-body");
        if (previewCardBody) {
  ...
  customerSearch.addEventListener("input", function() {
    ...
    fetch(`/customers/search?q=${encodeURIComponent(q)}`).then((res) => res.json()).then((data) => {
      resultsBox.innerHTML = "";
      if (data.length === 0) {
  ...
  function handleRoomChange() {
    ...
    fetch(`/bookings/check_room_status?room_id=${roomSelect.value}&check_in=${checkIn.value}&check_out=${checkOut.value}`).then((res) => res.json()).then((data) => {
      ...
      if (data.booked) {
        roomStatus.innerHTML = '<span class="badge bg-danger">\u{1F534} Not Available</span>';
        ...
        roomStatus.innerHTML = '<span class="badge bg-success">\u{1F7E2} Available</span>';
        if (isNewBooking && saveBtn) {
          ...
          <div class="card mb-2 border-0" style="background-color: #f8f9fa;">
            <div class="card-body py-2 px-3">
              <div class="d-flex justify-content-between align-items-start">
                ...
                <span class="badge ${b.status === "booked" ? "bg-primary" : b.status === "checked_in" ? "bg-success" : b.status === "checked_out" ? "bg-secondary" : b.status === "cancelled" ? "bg-danger" : b.status === "invoiced" ? "bg-info" : "bg-secondary"}">
                  ${b.status}
                </span>
              </div>
            </div>
